<template>
	<view class="discount">
		...
		<view class="discount-top">
			<!-- 头部的图 -->
			<!-- <image class="discount-top-img" src="../../../static/images/discount.png"></image> -->
		...
		<view class="discount-data" v-if="shopData.length > 0">
			...
			<view class="discount-data-detail" v-for="(shop, shopIndex) in shopData" :key="shopIndex"
				...
				<!-- <view class="discount-data-detail-shop">
					<view class="name">
						<view class="tri" :style="{borderLeft: `10rpx solid ${themeColor}`}"></view><view class="shopname">{{shop.shopName}}</view>
					</view>
					<view class="distance"><view class="address">{{shop.shopAddress}}</view> | <view class="far">{{shop.shopDistance}}km</view></view>
				</view> -->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			getDiscountType(typeParams).then(res => {
				if (res.object.length !== 0 && res.object) {
					//    this.tabList = res.object.map(m => {
					//    return {
					//      name: m.name,
					//      paths: m.paths || ''
					//      }
					//   })
					this.path = this.tabList[0].paths
					this.params = {
						regionNo: this.$regionId,
						categoryRegionPath: this.path,
						pager: {
							curPageNum: 1,
							pageSize: 10
						}
		...
		methods: {
			changeTab(index) {
				// 切换tab，则需要将shopData为空，修改参数
				// this.shopData = []
				this.current = index;
				// this.path = this.tabList[index].paths
				// this.params = {
				// 	regionNo: this.$regionId,
				// 	categoryRegionPath: this.path,
				// 	pager: {
			...
			getDetail() {
				...
				setTimeout(() => {
					getPreSettleCalc(this.params).then(res => {
						// this.record.status === 'loading'
						this.record = res.pager
						// this.shopData = res.object
						...
						if (this.record.curPageNum <= 1) {
							this.shopData = res.object || []
						} else if (this.record.pageSize * this.record.curPageNum >= this.shopData.length) {
							this.shopData = this.shopData.concat(res.object)
						}
			...
			onReachbottom() {
				...
				if (this.record.status === 'loading' || this.record.loaded && (this.shopData == null || this.shopData
						.length === 0)) {
					return
				}
				if (this.record.curPageNum >= this.record.totalPageNum) {
			...
			searchData() {
				...
				//   // url: '/pages/user/user-wallet-password'
				// })
			},
			toCouponDetail(item) {
				uni.navigateTo({
