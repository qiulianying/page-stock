<template>
	...
	<view class="user-user-bankcard-add-page">
		...
		<view class="page-box flex flex-direction" style="background: white;height: calc(100vh - 180rpx);">
          ...
          <view class="page-box-tip">输入卡号添加</view>
					<view class="page-box-input">
						...
						<input type="number" v-model="mediumNo" focus :maxlength="24"
...
<script>
	export default {
		...
		computed: {
			nextDisabled() {
				return this.$util.strIsEmpty(this.mediumNo.replace(/\s+/g, "")) || this.mediumNo.replace(/\s+/g, "")
					.length < 16 || !this.isBankNo
			}
		},
		onLoad() {
			this.themeColor = uni.getStorageSync('themeColor') || '#34A2E8'
			//判断当前缓存是否存在银行卡号，如果存在，就获取
			if (uni.getStorageSync('cardOcr') && uni.getStorageSync('cardOcr').cardNumber) {
				this.isBankNo = true
				this.mediumNo = uni.getStorageSync('cardOcr').cardNumber
			}
		},
		watch:{
		...
		methods: {
			doNextStep() {
				this.$toView(`user/user-bankcard-add-userinfo`, true)
			},
			//银行卡拍摄
			chooseCardImg() {
				uni.navigateTo({
