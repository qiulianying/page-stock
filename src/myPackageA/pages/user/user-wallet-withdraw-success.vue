<template>
	<view class="user-wallet-withdraw-success-page">
		<cu-custom bgColor="bg-white" is-back back-reload>
			<block slot="backText" class="text-black">成功提现</block>
		</cu-custom>
		<view v-if="visible.page" class="page-box flex flex-direction ">
			<view class="flex">
				...
				<view class="action-text">
					<view class="action-text-item text-gray">发起提现申请</view>
			...
			<view class="flex flex-direction info">
				<view class="action-info">
					<text class="action-info-title">提现金额</text>
					<text class="action-info-detail">¥ {{ recharge }}</text>
				</view>
				<view class="action-info">
					<text class="action-info-title">支付银行卡</text>
					<text class="action-info-detail">交通银行 尾号7272</text>
				</view>
			</view>
			<view class="flex" style="width: 100%; margin-top: 120rpx;">
				<button class="cu-btn page-box-btn bg-theme" @tap="$util.toBack(1, true)">完成</button>
			</view>

			<!-- <image src="/static/images/success.png"></image>
			<text style="font-size: 34rpx; margin: 24rpx 0;">提现成功！</text>
			<text style="font-size: 30rpx; color: #666666;">您已成功提现<text class="text-red">{{ recharge }}</text>元</text>
			<text style="font-size: 30rpx;">账户余额：¥ {{ $util.toMoney(balance / 100) }}</text>
			<view class="flex" style="width: 100%; margin-top: 120rpx;">
				<button class="cu-btn page-box-btn bg-theme" :style="{background: themeColor}" @tap="$util.toBack(1, true)">完成</button>
			</view> -->
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async queryAccBalance() {
				await queryAccBalance({
					...
					this.balance = obj.accountBalance
				})
			}
		},
		async onLoad(option) {
...
<style lang="scss">
	.user-wallet-withdraw-success-page {
		.page-box {
